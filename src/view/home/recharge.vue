                    ...
                    <ul class="priceul">
                        <li @click="ps(index,item.vip_id)" v-for="(item,index) in vipList" :key="index">
                            ...
                            <div class="priceulright">
                                <div>￥{{item.price}}</div>
                                <div class="rrr">￥{{item.s_price}}</div>
                            </div>
...
export default {
    ...
    methods:{
        ...
        pays(){
            ...
            _this.$hapi.orderPay({p_source:parseInt(_this.p_source),p_way:parseInt(_this.pricetype+1),three_id:parseInt(ids)}).then(function(response) {
                let data = response.data;
                if(data.code == 200){
                    _this.spinShow = false;
